...
export default function Navbar() {
  ...
  const [hoveredIndustry, setHoveredIndustry] = useState<Industry>(industries[0]);
  ...
  const closeIndustries = () => {
    industryCloseTimer.current = setTimeout(() => setIndustriesOpen(false), 150);
  };
    ...
    >
      <div className="px-6 lg:px-8 py-5">
        <div className="flex gap-8">
          {/* Left – 4-col industry grid with sub-items */}
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 pb-2 border-b border-gray-100">
              Industries We Serve
            </p>
            <div className="grid grid-cols-4 gap-x-4">
              {[industries.slice(0, 2), industries.slice(2, 4), industries.slice(4, 6), industries.slice(6)].map((col, colIdx) => (
                <div key={colIdx} className="space-y-2">
                  {col.map((industry) => (
                    <div key={industry.id} className="p-2 shadow-sm shadow-gray-400 rounded-lg">
                      <Link
                        ...
                        className={`flex items-center gap-2 py-1.5 px-2 rounded-lg group transition-colors ${
                          hoveredIndustry.id === industry.id ? 'bg-teal/5' : 'hover:bg-gray-50'
                        }`}
                      >
                        <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-all duration-150 ${
                          hoveredIndustry.id === industry.id ? 'bg-teal text-white' : 'bg-navy text-white group-hover:bg-teal'
                        }`}>
                          <Icon name={industry.icon} size={12} className="text-white" />
                        </div>
                        <span className={`font-semibold text-sm transition-colors duration-150 ${
                          hoveredIndustry.id === industry.id ? 'text-teal-dark' : 'text-navy group-hover:text-teal-dark'
                        }`}>
                          {industry.title}
                        </span>
                      </Link>
                      {industry.subItems && (
                        <ul className="ml-8 mt-0.5 space-y-0.5">
                          {industry.subItems.map((sub) => (
                            <li key={sub.href}>
                              <Link
                                href={sub.href}
                                onMouseEnter={() => setHoveredIndustry(industry)}
                                onClick={() => setIndustriesOpen(false)}
                                className="block text-xs py-0.5 px-1.5 rounded transition-colors hover:text-teal-dark hover:bg-teal/5"
                              >
                                {sub.title}
                              </Link>
                            </li>
          ...
          {/* Right – hovered industry preview */}
  ...
  const desktopLinks = (compact: boolean) => (
    <div className="hidden min-[1114px]:flex items-center gap-6">
      ...
      <button
        ...
        onMouseLeave={closeIndustries}
        className={`flex items-center gap-1 font-medium text-sm whitespace-nowrap transition-colors duration-200 ${
          isIndustriesActive ? 'text-teal-dark border-b-2 border-teal' : 'text-navy hover:text-teal'
        }`}
        aria-haspopup="true"
  ...
  const mobileMenuContent = (
    <div className="flex flex-col space-y-1">
      ...
      <button
        ref={mobileIndustriesRef}
        onClick={() => {
          const next = !mobileIndustriesOpen;
          setMobileIndustriesOpen(next);
          if (next) {
            setTimeout(() => mobileIndustriesRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50);
          }
        }}
        className={`w-full flex items-center justify-between py-2.5 px-3 rounded-lg text-sm font-medium transition-colors text-gray-900 hover:bg-teal/5 ${
          isIndustriesActive ? 'border-l-4 border-teal bg-teal/5' : ''
        }`}
      >
        Industries
        <svg className={`w-4 h-4 transition-transform duration-200 ${mobileIndustriesOpen ? 'rotate-180' : ''}`}
          fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      ...
      {mobileIndustriesOpen && (
        <div className="ml-3 border-l-2 border-teal/20 pl-3 space-y-1">
          <Link href="/industries" onClick={() => { closeMenu(); setMobileIndustriesOpen(false); }}
            className="block py-2 px-2 text-sm text-teal-dark font-semibold hover:bg-teal/5 rounded-lg">
            All Industries
          </Link>
          {industries.map((ind) => (
            <div key={ind.id}>
              <Link href={`/industries/${ind.slug}`} onClick={() => { closeMenu(); setMobileIndustriesOpen(false); }}
                className="flex items-center gap-2 py-2 px-2 text-sm text-navy hover:text-teal hover:bg-teal/5 rounded-lg font-medium">
                <Icon name={ind.icon} size={14} className="text-teal-dark" />
                {ind.title}
              </Link>
              {ind.subItems && (
                <ul className="ml-7 mt-0.5 space-y-0.5">
                  {ind.subItems.map((sub) => (
                    <li key={sub.href}>
                      <Link
                        href={sub.href}
                        onClick={() => { closeMenu(); setMobileIndustriesOpen(false); }}
                        className="block py-1 px-2 text-xs hover:text-teal-dark hover:bg-teal/5 rounded"
                      >
                        {sub.title}
                      </Link>
  ...
  return (
    <>
      ...
      <nav style={{ willChange: 'transform' }} className={`fixed top-0 left-0 right-0 z-50 transition-transform duration-300 ${isScrolled ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        {/* Industries dropdown — sticky nav */}
        {industriesOpen && isScrolled && industriesDropdown}
      </nav>
      ...
      <nav className="relative top-0 z-40 px-2 bg-[#0a1e3c] pt-4">
        ...
        {servicesOpen && !isScrolled && servicesDropdown}
        {/* Industries dropdown — original nav */}
        {industriesOpen && !isScrolled && industriesDropdown}
      </nav>
    </>
  );
